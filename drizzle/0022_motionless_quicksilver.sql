...
ALTER TABLE `purchaseOrderExtractionReviews`
ADD `catalogResolutionsJson` text;
--> statement-breakpoint

ALTER TABLE `purchaseOrderItems`
ADD `catalogItemId` varchar(50);
--> statement-breakpoint
...
CREATE INDEX `catalogItemAliases_catalogItem_active_idx`
ON `catalogItemAliases` (`catalogItemId`,`active`);
--> statement-breakpoint

CREATE INDEX `catalogItems_active_normalizedName_idx`
ON `catalogItems` (`active`,`normalizedName`);