...
;
ALTER TABLE `rolePermissions` MODIFY COLUMN `updatedAt` timestamp DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP;
;
...
;
ALTER TABLE `enquiries` MODIFY COLUMN `createdAt` timestamp NOT NULL DEFAULT (now());
;
ALTER TABLE `externalApiAuditLogs` MODIFY COLUMN `timestamp` timestamp NOT NULL DEFAULT (now());
;
