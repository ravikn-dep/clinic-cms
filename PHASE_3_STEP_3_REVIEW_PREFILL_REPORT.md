...
The corrective implementation commit contains actual frontend source changes in `client/src/pages/PurchaseOrders.tsx`, plus a shared model and dedicated test file. It is not a documentation-only candidate. The required final Git evidence is collected after this report commit and before GitHub publication.

## Classification

**`PHASE3_STEP3_REVIEW_PREFILL_IMPLEMENTED`** — local implementation and validation completed. This status may be upgraded to `PHASE3_STEP3_REVIEW_PREFILL_GITHUB_VALIDATED` only after the canonical pull request is pushed and its required `validate` check is green.
