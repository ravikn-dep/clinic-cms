 ...
 * documented otherwise. ctx.user is hydrated from the signed session cookie.
      ...
      .mutation(async ({ input, ctx }) => {
        try {
          ...
          await safeNotifyOwner(
            ...
            `${input.name} (${userId}) has been added to the system. Username: ${username}, temporary password: ${tempPassword}`
          );
